...
- **Derived dimensions** — the `Derive.BorderThickness`, `Derive.CornerRadius` and `Derive.Margin` attached properties (plus the `Derive.Scaler` converter for an overridable control property), with their factors scaling the value side by side, for thicknesses, radii, margins and paddings that follow the theme live.
