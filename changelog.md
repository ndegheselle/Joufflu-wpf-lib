...
- Add `Derive.Base`, `Derive.Factor` and the `Derive.Scaler` converter, and give the text inputs — `TextBox`, `PasswordBox` and `FormatTextBox` — a padding derived from the control padding at half its horizontal, tighter than a button by design without a hardcoded thickness. Delivered through the converter in a plain `Padding` setter rather than pushed with `SetCurrentValue`, so it lands at style setter precedence and a consumer `Padding` still wins, while every size variant is covered where the base text input padding used to be fixed whatever the size
